...
#endif
	size_t  r;
	u64b_t  __attribute__((aligned(16))) kw[WCNT+4+RCNT*2];                  /* key schedule words : chaining vars + tweak + "rotation"*/
#else
	u64b_t  __attribute__((aligned(16))) kw[WCNT+4];                         /* key schedule words : chaining vars + tweak */
#endif
	u64b_t  __attribute__((aligned(16))) X[4];                        /* local copy of context vars, for speed */
...
#define I256(R)                                                     \
	X[0]   += ks[((R)+1) % 5];    /* inject the key schedule value */ \
	X[1]   += ks[((R)+2) % 5] + ts[((R)+1) % 3];                      \
	X[2]   += ks[((R)+3) % 5] + ts[((R)+2) % 3];                      \
	X[3]   += ks[((R)+4) % 5] +     (R)+1;                            \
